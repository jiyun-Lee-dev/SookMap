...
public class UpdateNavigation : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.name.Equals("NavTrigger(Clone)") && line.positionCount > 0)
        {
            ...
            Vector3 pos = arcoreDeviceCam.transform.position + arcoreDeviceCam.transform.forward * 4 + arcoreDeviceCam.transform.up * -1f;
            // rotate arrow a bit
        ...
        if (other.tag.Equals("destination") && line.positionCount > 0 && GameObject.Find("PathShower").GetComponent<NavigationController>().target.name.Equals(other.name))
        {
            ...
            Vector3 pos = arcoreDeviceCam.transform.position + arcoreDeviceCam.transform.forward * 1 + arcoreDeviceCam.transform.up * -0.3f;
